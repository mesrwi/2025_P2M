...
def main():
    ...
    with tqdm(total=int(vidcap.get(cv2.CAP_PROP_FRAME_COUNT)), ncols=100, desc="Processing frames") as pbar:
        while vidcap.isOpened():
            ...
            # 프레임 리사이즈+패딩
            processed_frame, scale, offset = resize_with_padding(image_bgr, target_size=512)
            ...
            pose2d_dict = {"image_id": 'pose_{:08d}.jpg'.format(count), "keypoints": pose_preds[0].tolist()}
            pose2d.append(pose2d_dict)
            
            csv_output_rows.append(new_csv_row)
            img_file = os.path.join(pose_dir, 'frames', 'pose_{:08d}.jpg'.format(count))
            cv2.imwrite(img_file, image_debug)
            outcap.write(image_debug)
            
            pbar.update(1)
    
    pose2d_filename = os.path.join(args.outputDir, 'pose2d.json')
    with open(pose2d_filename, 'w') as jsonfile:
        json.dump(pose2d, jsonfile, indent=4)
